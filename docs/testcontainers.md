...
| | |
|---|---|
| Image | `ghcr.io/nubo-db/dynoxide:1.2` |
| Port | `8000` |
...
Pin to `:1.2` rather than `:1` for a test fixture. Both float, but `:1` picks up
...
GenericContainer<?> dynoxide = new GenericContainer<>(
        DockerImageName.parse("ghcr.io/nubo-db/dynoxide:1.2"))
...
```ts
const dynoxide = await new GenericContainer("ghcr.io/nubo-db/dynoxide:1.2")
  .withExposedPorts(8000)
...
```go
ctr, err := testcontainers.Run(ctx, "ghcr.io/nubo-db/dynoxide:1.2",
...
var dynoxide = new ContainerBuilder()
    .WithImage("ghcr.io/nubo-db/dynoxide:1.2")
