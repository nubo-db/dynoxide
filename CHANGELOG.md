...
- The wasm build serves the vector surface. `SearchVectors` is routed rather than answering the preview "not supported" error, and the facade gains `search_vectors`. The control plane could already create a vector index and reflect it on `DescribeTable`, because the engine logic and all six vector storage methods are shared with native, so until now a wasm build would report an index it could not search. A database persisted before the vector column existed is migrated on open, so an OPFS database from an older build gains the surface without being recreated.

- `SearchVectors` and the write paths report vector `ConsumedCapacity`. A search carries `VectorSearchRequestBytes` and nothing else: no `CapacityUnits`, no `TableName`, identical under `TOTAL` and `INDEXES`, absent under `NONE`. A write to a vector-indexed table gains a `VectorIndexes` map keyed by index name carrying `VectorWriteRequestBytes`, under `INDEXES` alone, and `TOTAL` carries no vector fields at all. Vector figures are bytes rather than capacity units, so they never join `CapacityUnits` and never take the transactional 2x factor. Replication is delta-based on the same terms as the classic indexes: an index the write leaves untouched is absent from the map rather than present and zeroed, an identical overwrite reports no map, and re-listing a set's members costs nothing. A write that leaves the vector alone still charges an index that projects the attribute it did change, and charges nothing on an index that does not. A write that changes only the vector charges every index over it, `KEYS_ONLY` included, because the entry holds the vector whatever the projection. Both shapes captured against eu-west-2, as is the 1KB floor applying per item and then summing, so a three-item batch reports 3072 rather than 1024.

  A write's figure is `4 * dimensions` plus the vector attribute's name plus the item size of the rest of the projected entry, held to a 1KB floor. The vector is billed at its f32 width rather than as the decimal text it stores as, and is counted once even where the projection also carries it. Captured byte-exact against eu-west-2 across five fixtures from 3 to 512 dimensions.

  A search's figure is sized on the index entries it scanned, with the same floor, using the same captured measure as a write. This is a deliberate divergence: real DynamoDB bills a search on the data it read rather than on the request, and does not repeat the number, with five identical searches over one unchanged index reporting 14214, 13903, 14214, 14214 and 14518. Dynoxide reports the same quantity deterministically, in the same class as the equal-score tie break. The unit matches the captured write formula, but the quantity is still Dynoxide's own and not something to compare against AWS.

- **Breaking (behaviour):** `SearchVectors` refuses two condition shapes it used to answer with an empty result set. A condition on a nested attribute now gives `SearchConditionExpression cannot have conditions on nested attributes`, and more than one condition on the same attribute gives `SearchConditionExpression must only contain one condition per attribute`. The second applies whether the conditions agree, contradict, or carry a type error between them, and the order they appear in does not change it, so an engine reconciling two scope values was modelling something the wire never accepts. A malformed search used to be indistinguishable from one that matched nothing. Captured against eu-west-2.

- **Breaking (behaviour):** a vector index whose `VectorAttribute.AttributeName` is empty is refused rather than created. The API model bounds it at one character. An empty name produced an index that reported `ACTIVE` and could never hold a row, because no item can carry an attribute with no name.

- A `Create` or `Delete` inside `VectorIndexUpdates` that is not a structure now names the vector action in its `SerializationException` rather than the global secondary index one. The two share a field name and only the parent decides the type.

- **Breaking (behaviour):** `Query` and `Scan` naming a vector index are refused by type rather than reported missing, matching the PartiQL reader. Both used to answer `The table does not have the specified index`, for an index `DescribeTable` lists. `Query` now answers `Query operation not supported on this index type.` and `Scan` answers `Scan operation not supported on this index type`; the trailing full stop is present on one and absent on the other, which is AWS's own inconsistency and captured that way. A consistent read against a vector index keeps the existing wording, which names global secondary indexes even though the index is not one; that is also what DynamoDB says. Captured against eu-west-2.

- The wasm surface runs the same pre-deserialisation type checks as the HTTP server, so a mistyped field answers with the captured `SerializationException` on both instead of raw serde text on one. The checks moved out of the server module into one both surfaces read; they were previously unreachable from the wasm build, which affected every operation with such a check and not only the vector ones.

- **Breaking (behaviour):** a PartiQL `SELECT` naming a vector index in a `"table"."index"` qualifier is refused by type rather than reported missing. It used to answer `The table does not have the specified index`, which was wrong twice over: the index exists, and the reason no read can reach it is its type. It now answers `Scan operation not supported on this index type`, as DynamoDB does. A write naming any index keeps its own rejection, which fires before the table is resolved and is unchanged. Captured against eu-west-2.
...
- **Breaking (Rust API):** three public structs gained a field, so code constructing any of them with a struct literal needs `..Default::default()` or the existing constructors: `types::ConsumedCapacity` gained `vector_indexes`, and `actions::index_capacity::WriteCapacity` and `actions::index_capacity::TableCapacity` each gained `vector_bytes`. `actions::search_vectors::SearchVectorsResponse` gained `consumed_capacity` on the same terms. Reading any of them is unaffected. New public items: `types::VectorCapacityDetail`, `types::VectorSearchCapacity`, `types::MIN_VECTOR_REQUEST_BYTES`, `types::vector_request_bytes`, `types::consumed_capacity_with_vector_indexes`, and `actions::index_capacity::WriteCapacity::with_vector_bytes`. The DynamoDB wire API is unaffected beyond the new arms described above.

- **Breaking (Rust API):** the `StorageBackend` trait gained seven required methods for vector index support. `vector_items_for_keys` loads the projected entries for the rows a search returns, and pairs with `query_vector_candidates`, which deliberately does not carry them: a search scores from the vector and the filter alone, so a scan of a large index no longer materialises every projected entry only to drop it. `VectorItemRow` and `VectorCandidateRow` carry the entry's billable size, computed once per write, so a search sums a column rather than rebuilding what it scanned; `VectorCandidateRow` no longer carries `item_json`. The other six: `create_vector_table` and `drop_vector_table` manage a vector index's physical shadow table, `update_vector_index_definitions` writes a table's stored vector index definitions, `insert_vector_items` bulk-inserts backfill rows, `delete_vector_item` removes an item's rows across a table's vector indexes, and `query_vector_candidates` loads the rows a search scores. Two new public structs come with them: `VectorItemRow` for the rows written and `VectorCandidateRow` for the rows read back. Backend implementations outside the crate must add all six; the bundled rusqlite and wasm backends already have them. The DynamoDB wire API and the CLI/server/MCP surfaces are unaffected.

### Fixed

- The write paths no longer disagree about the 400KB item-size limit ([#187](https://github.com/nubo-db/dynoxide/issues/187)). A number was sized by counting the digit characters of whatever string it held, and storage expands scientific notation, so the same item measured one size as it arrived and another once stored. `PutItem` checked the limit before that expansion and the other three after it, which is how an item of 8000 attributes holding `N "1E125"` passed `PutItem`, was refused by `BatchWriteItem`, and was then recorded at 552,004 bytes against a 409,600 limit. The recorded size feeds table statistics and the `ItemCollectionMetrics` estimate. Sizing a number by its significant digits makes the figure survive storage, so where each path checks no longer matters. The surfaces that measure differently on purpose, and the two that were not checking at all, are covered in the behaviour changes above.
