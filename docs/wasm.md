...
**What doesn't, yet:** `SearchVectors` is native-only for now and returns the preview "not supported" error on wasm. Mind the asymmetry while that is true: `CreateTable` does accept `VectorIndexes` here, the index is created, and `DescribeTable` reports it `ACTIVE`, so the wasm build will currently tell you it has a vector index it cannot search. The engine logic and the storage backend are already shared, so wiring the search side is dispatch work rather than a capability gap, and until it lands treat vector indexes as unsupported on wasm regardless of what `DescribeTable` says. TTL returns a typed `Unsupported` error (it needs a background sweep the browser doesn't drive). Streams are planned but not wired - the delivery mechanism is still to be decided, so an `UpdateTable` that changes a stream specification is refused. `CreateTable` applies the same rule up front: a request carrying an enabled stream specification or tags is refused before anything is created, so a refusal never leaves a half-made table. `TransactWriteItems`, tags (on `CreateTable` as well as the `TagResource` family), table stats, and bulk import return a preview "not yet implemented" error. Those four blocks are the whole of the conformance skip count.
