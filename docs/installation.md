...
# Or with encryption:
# dynoxide-rs = { version = "2.0", default-features = false, features = ["encryption"] }
```
...
```yaml
- uses: nubo-db/dynoxide/action@v1.2.0
  with:
...
Image tags follow the version: `dynoxide:1.2.0` is exact, `dynoxide:1.2` floats
across patches, and `dynoxide:1` floats across the whole 1.x line. `:1` receives
conformance fixes, which change behaviour within a major; `:1.2` does not. See
`docs/versioning.md`.
