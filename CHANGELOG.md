...
- `SearchVectors` and the write paths report vector `ConsumedCapacity`. A search carries `VectorSearchRequestBytes` and nothing else: no `CapacityUnits`, no `TableName`, identical under `TOTAL` and `INDEXES`, absent under `NONE`. A write to a vector-indexed table gains a `VectorIndexes` map keyed by index name carrying `VectorWriteRequestBytes`, under `INDEXES` alone, and `TOTAL` carries no vector fields at all. Vector figures are bytes against a 1KB floor rather than capacity units, so they never join `CapacityUnits` and never take the transactional 2x factor. Replication is delta-based on the same terms as the classic indexes: an index the write leaves untouched is absent from the map rather than present and zeroed, an identical overwrite reports no map, and re-listing a set's members costs nothing. Captured against eu-west-2. Magnitudes above the floor are not pinned by the capture, so the per-byte model is documented rather than measured.
...
- **Breaking (Rust API):** `types::ConsumedCapacity` gained a `vector_indexes` field and `actions::index_capacity::WriteCapacity` a `vector_bytes` one, so code constructing either with a struct literal needs `..Default::default()` or the existing constructors. Two new public types come with them, `types::VectorCapacityDetail` and `types::VectorSearchCapacity`, alongside `types::vector_request_bytes` and `types::attach_vector_index_capacity`. Reading a response is unaffected. `SearchVectorsResponse` gained an optional `consumed_capacity`. The DynamoDB wire API is unaffected beyond the new arms described above.
