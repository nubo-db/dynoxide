...
- `UpdateItem` measures the 400KB limit the way DynamoDB does, which is not the way a put is measured ([#187](https://github.com/nubo-db/dynoxide/issues/187)). It applied the same ceiling to the finished item as `PutItem`, where DynamoDB leaves the key attributes out of the figure and charges a fixed amount per action. Captured against eu-west-2: three bytes for the update, nineteen for each action that writes a value, two for each that clears one, and nothing for the value written, the attribute name, or the expression text. With a one-character key that puts an update's ceiling nineteen bytes below a put's, so an item can be reachable by `PutItem` and out of reach by `UpdateItem`. Updates in that band used to be accepted and now fail.
